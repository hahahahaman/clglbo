...
(defun handle-player-input ()
  (flet ((move-player (components)
           (let ((input (get-component :player-input components))
                 ;; (vel (get-component :vel components))
                 ;; (accel (get-component :accel components))
                 (pos (get-component :pos components)))
             (cond ((and input pos)
                    (when (key-pressed-p :left)
                      (set-component :pos
                                     (vec-add pos (vec2 (cfloat (- (* 200.0 *dt*))) 0.0))
                                     components))
                    (when (key-pressed-p :right)
                      (set-component :pos
                                     (vec-add pos (vec2 (cfloat (* 200.0 *dt*)) 0.0))
                                     components))
                    components)
                   (t components)))))
    (setf *entities* (mapcar #'move-player *entities*))))
